...
export default function TradingChart({ symbol: propSymbol, segment: propSegment = '', liveQuote: propLiveQuote }: TradingChartProps) {
  ...
  return (
    ...
    >
      ...
      <div 
        className={`unified-order-panel ${(isLandscape || isCssLandscape) && !isPanelExpanded ? 'drawer-hidden' : ''}`}
        style={{
          display: 'flex',
          ...
          ...((isLandscape || isCssLandscape) ? {
             ...
             width: '100%',
             borderTop: '1px solid var(--border-color, #2B3139)'
          })
        }}
      >
        <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', height: '100%', gap: '16px' }}>
          ...
          <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around', borderTop: '1px solid #eaecef', paddingTop: '12px', marginTop: '4px', color: '#787B86' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => { setActiveSegment('positions'); setIsInfoPanelExpanded(true); }}>
                <i className="ti ti-briefcase" style={{ fontSize: '20px' }}></i>
                <span style={{ fontSize: '10px' }}>Portfolio</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', cursor: 'pointer' }} onClick={() => { setActiveSegment('chain'); setIsInfoPanelExpanded(true); }}>
      ...
      {/* Info Panel */}
      <div className={`info-panel ${!isInfoPanelExpanded ? 'collapsed' : ''}`} id="infoPanel" style={{ zIndex: 1000, position: 'absolute', bottom: 0, left: 0, right: 0, maxHeight: '60vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'flex-end', padding: '8px', background: '#F8F9FA', borderBottom: '1px solid #eaecef' }}>
          <button onClick={() => setIsInfoPanelExpanded(false)} style={{ background: 'transparent', border: 'none', color: '#1E222D', cursor: 'pointer' }}>
            <i className="ti ti-x" style={{ fontSize: '20px' }}></i>
          </button>
        </div>
        <div className={`panel-content ${activeSegment === 'chain' ? 'chain-mode' : ''}`} style={{ background: '#ffffff' }}>
          {renderPanelContent()}
        </div>
      </div>

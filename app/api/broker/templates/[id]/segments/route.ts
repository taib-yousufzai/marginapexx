/**
 * GET  /api/broker/templates/[id]/segments  — get segment settings for a template
 * POST /api/broker/templates/[id]/segments  — upsert segment settings for a template
 *
...
export async function GET(
  ...
  try {
    ...
    // Check ownership
    const { data: tmpl, error: tmplErr } = await brokerClient.from('account_templates').select('created_by').eq('id', id).single();
    if (tmplErr || tmpl.created_by !== callerUser.id) {
...
export async function POST(
  ...
  try {
    ...
    if (error) {
      console.error(`[POST /api/broker/templates/${id}/segments] ${error.message}`);
      return Response.json({ error: 'Internal server error' }, { status: 500 });
    }

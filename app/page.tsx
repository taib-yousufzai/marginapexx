...
const getExpiryIndexes = () => [
  ...
  { name: "BANK NIFTY", fullName: "BANK NIFTY", expiry: getNextExpiryDate(3), lotSize: 15, icon: "fas fa-building" },
  { name: "SENSEX", fullName: "SENSEX", expiry: getNextExpiryDate(5), lotSize: 10, icon: "fas fa-chart-area" },
  { name: "BANKEX", fullName: "BANKEX", expiry: getNextExpiryDate(5), lotSize: 15, icon: "fas fa-university" },
  { name: "FIN NIFTY", fullName: "FINNIFTY", expiry: getNextExpiryDate(2), lotSize: 40, icon: "fas fa-chart-pie" },
  { name: "MIDCAP NIFTY", fullName: "MIDCAP NIFTY", expiry: getNextExpiryDate(1), lotSize: 75, icon: "fas fa-chart-bar" }
];
          ...
          <div className="expiry-half-sheet">
            ...
            <div className="expiry-list">
              {getExpiryIndexes().map((item, i) => (
                <div className="expiry-list-item" key={i} onClick={() => router.push(`/option-chain?symbol=${encodeURIComponent(item.name)}`)}>
                  <div className="expiry-info">
                    <h4><i className={item.icon}></i> {item.name}</h4>
                    <p>📅 Expiry: {item.expiry.isToday ? <span className="today-badge">Today</span> : item.expiry.dateStr} • Lot: {item.lotSize}</p>
                  </div>

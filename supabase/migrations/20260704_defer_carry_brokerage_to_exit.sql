-- Migration: Defer carry brokerage from entry-time to exit-time
--
-- Problem: When users switch between INTRADAY and CARRY, brokerage was double-charged
-- or missed because carry brokerage was charged upfront at entry.
--
-- Solution: Only charge intraday + GTT brokerage at entry. Carry brokerage is now
-- calculated and charged at exit-time via close_position() RPC's p_brokerage parameter.
--
-- This migration updates the process_executed_position() function to remove carry
-- brokerage from the entry-time calculation.
